...
class ModemConfig:
    ...
    # Receiver-side tuning only.  These values do not affect the transmitted
    # waveform or any interoperability-critical bitstream.
    sync_refine_radius: int = 512
    sync_min_chirps: int = 6
    sync_peak_search_radius: int = 192
    sync_max_sfo_ppm: float = 5_000.0
    sync_max_fit_residual: float = 8.0
    resampler_half_width: int = 16
    ...
    pilot_min_snr_db: float = 3.0
    pilot_max_relative_jump: float = 8.0
    decision_phase_limit_deg: float = 25.0
    decision_delay_limit_samples: float = 0.35
    ldpc_rescue_timing_offsets: tuple[float, ...] = (-0.75, -0.375, 0.0, 0.375, 0.75)
...
# pilot_data.py
# -----------------------------------------------------------------------------
# Official standardisation file containing the 854 valid-band QPSK points used
# by every known OFDM pilot block. The file must sit beside modem.py.
KNOWN_PILOT_FILENAME = "spliced_seed_qpsk.npy"
KNOWN_PILOT_SHA256 = "d6296e923d98f30edf1a518ae981ce50f7f79827abed6f7a24bf6413b294c76b"
...
class LinearChirp:
    ...
    def samples(self) -> NDArray[np.float64]:
        c = self.config
        t = np.linspace(0.0, c.chirp_length / c.sample_rate, c.chirp_length)
        return c.chirp_amplitude * chirp(
            t,
            f0=c.chirp_start_hz,
            f1=c.chirp_stop_hz,
            t1=c.chirp_length / c.sample_rate,
            method="linear",
        )
...
class KnownOFDMPilot:
    """JOSS-F known OFDM pilot loaded from ``spliced_seed_qpsk.npy``.

    The standardisation file contains exactly one complex QPSK point for each
    of the 854 occupied bins, in ascending bin order. These points are placed
    on bins 171..1024 inclusive; all other rFFT bins are zero. The same file is
    therefore used identically by the transmitter and receiver.
    """

    def __init__(self, config: ModemConfig = CONFIG):
        self.config = config
        self.path = Path(__file__).resolve().parent / KNOWN_PILOT_FILENAME
        self.symbols = self._load_symbols()
        canonical = np.ascontiguousarray(self.symbols.astype(np.dtype("<c16"), copy=False))
        self.content_sha256 = hashlib.sha256(canonical.tobytes()).hexdigest()
        self.frequency = self._frequency_vector()
        self.time = np.fft.irfft(self.frequency, n=config.fft_length) * config.ofdm_scale
        # Reference the spectrum that is actually transmitted after irfft.
        self.transmitted_frequency = np.fft.rfft(self.time, n=config.fft_length)

    def _load_symbols(self) -> NDArray[np.complex128]:
        if not self.path.is_file():
            raise FileNotFoundError(
                f"JOSS-F known-pilot file not found: {self.path}. "
                f"Place {KNOWN_PILOT_FILENAME} in the same directory as modem.py."
            )

        loaded = np.load(self.path, allow_pickle=False)
        symbols = np.asarray(loaded)
        expected = (len(self.config.data_bins),)
        if symbols.shape != expected:
            raise ValueError(
                f"{KNOWN_PILOT_FILENAME} must have shape {expected}, "
                f"but has shape {symbols.shape}"
            )
        if not np.issubdtype(symbols.dtype, np.number):
            raise TypeError(
                f"{KNOWN_PILOT_FILENAME} must contain numeric complex QPSK values"
            )

        symbols = symbols.astype(np.complex128, copy=False)
        if not np.all(np.isfinite(symbols)):
            raise ValueError(f"{KNOWN_PILOT_FILENAME} contains non-finite values")

        # The agreed pilot consists of unnormalised JOSS-F QPSK points ±1±j.
        real_valid = np.isclose(np.abs(symbols.real), 1.0, atol=1e-12, rtol=0.0)
        imag_valid = np.isclose(np.abs(symbols.imag), 1.0, atol=1e-12, rtol=0.0)
        if not np.all(real_valid & imag_valid):
            bad = int(np.flatnonzero(~(real_valid & imag_valid))[0])
            raise ValueError(
                f"{KNOWN_PILOT_FILENAME}[{bad}]={symbols[bad]!r} is not a "
                "JOSS-F QPSK point (expected real and imaginary parts ±1)"
            )
        return symbols

    def _frequency_vector(self) -> NDArray[np.complex128]:
        spectrum = np.zeros(self.config.fft_length // 2 + 1, dtype=np.complex128)
        spectrum[self.config.data_bins] = self.symbols
        return spectrum
...
# coding.py
# -----------------------------------------------------------------------------
# Cohort/Jossy IEEE 802.16 rate-1/2 protograph.
#
# IMPORTANT: the supplied Jossy implementation uses these shift values directly
# and reduces them modulo Z.  It does NOT apply the scaling rule from the formal
# WiMAX standard when Z=61.  The plugfest bitstream is therefore defined by this
# de-facto implementation.  Scaling these values by Z/96 creates a different code
# that can self-decode but is not interoperable with groups using Jossy's code.
...
class WiMaxLDPC:
    """Jossy-compatible 802.16 rate-1/2 QC-LDPC implementation.

    The parity-check graph exactly follows the cohort-supplied ``ldpc.py``: the
    protograph entries above are used directly modulo Z.  This is intentionally
    different from scaling the Z0=96 entries by Z/96.
    """

    JOSSY_REFERENCE_CODEWORD_SHA256 = (
    ...
    def __init__(self, config: ModemConfig = CONFIG):
        ...
        self.assert_jossy_compatible()

    def _scaled_base(self) -> NDArray[np.int16]:
        """Return the exact protograph used by Jossy's supplied implementation.

        The historical method name is retained to avoid changing callers.  Do
        not scale the non-negative entries: Jossy's implementation applies each
        shift directly and NumPy's cyclic operations reduce it modulo ``Z``.
        """
        return _WIMAX_RATE_HALF_BASE.copy()

    def compatibility_fingerprint(self) -> str:
        """Return a deterministic codeword hash for cross-group verification."""
        probe = (
            ((np.arange(self.k, dtype=np.int64) * 73 + 19) % 101) < 50
        ).astype(np.uint8)
        codeword = self.encode_block(probe)
        return hashlib.sha256(np.packbits(codeword).tobytes()).hexdigest()

    def assert_jossy_compatible(self) -> None:
        """Fail immediately if the configured Z=61 code differs from Jossy's."""
        if self.z != 61:
            return
        actual = self.compatibility_fingerprint()
        if actual != self.JOSSY_REFERENCE_CODEWORD_SHA256:
            raise RuntimeError(
                "LDPC implementation is not compatible with Jossy's supplied "
                f"802.16 rate-1/2 Z=61 encoder: {actual}"
            )
