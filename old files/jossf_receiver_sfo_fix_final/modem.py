...
class ModemConfig:
    ...
    # Receiver-side tuning only.  None of these values changes the transmitted
    # waveform or any standardised bit-level behaviour.
    ...
    # Sampling-frequency-offset (SFO) estimation/correction.
    sfo_refinement_iterations: int = 2
    sfo_min_correction_ppm: float = 0.25
    sfo_max_residual_ppm: float = 1_500.0
    sfo_min_timing_observations: int = 8
    sfo_max_timing_error_samples: float = 512.0

    # Receiver diagnostics only.
    diagnostic_constellation_points: int = 12_000
